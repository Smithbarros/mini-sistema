#!bin/bash/
  E='echo -e';e='echo -en';trap "R;exit" 2
    ...
    HEAD(){ for each in $(seq 2 16);do
           ...
           $E "               5º questão                 " ;UNMARK;
           TPUT 2 5
           $E "Elaborar um script que receba um nome de
   |arquivo e verifique se esse arquivo 
   |existe. Em caso afirmativo, imprimir
   |o seu tamanho em bytes. " ;UNMARK;}
  ...
  ARROW(){ IFS= read -s -n1 key 2>/dev/null >&2
     ...
     M0(){ TPUT  10 12; $e "Abrir a questao 05      ";}
...
REFRESH(){ after=$((i+1)); before=$((i-1))
     ...
     while [[ "$O" != " " ]]; do case $i in
        0) S=M0;SC;if [[ $cur == enter ]];then R; /bin/bash 05_quest.sh;ES; fi;;
