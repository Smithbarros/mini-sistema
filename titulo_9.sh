#!bin/bash/
  E='echo -e';e='echo -en';trap "R;exit" 2
    ...
    HEAD(){ for each in $(seq 2 16);do
           ...
           $E "               9º questão                 " ;UNMARK;
           TPUT 2 5
           $E "Desenvolva um script que receba uma lista
   |de palavras como parâmetro e exiba, como
   |resultado, essas palavras ordenadas." ;UNMARK;}
  ...
  ARROW(){ IFS= read -s -n1 key 2>/dev/null >&2
     ...
     M0(){ TPUT  10 12; $e "Abrir a questao 09      ";}
...
REFRESH(){ after=$((i+1)); before=$((i-1))
     ...
     while [[ "$O" != " " ]]; do case $i in
        0) S=M0;SC;if [[ $cur == enter ]];then R; /bin/bash 09_quest.sh;ES; fi;;
